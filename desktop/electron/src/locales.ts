...
const messages: Record<DesktopLocale, DesktopMessages> = {
  en: {
    startupFailureTitle: "Personal-Trading failed to start",
    backendAlreadyRunning: "The backend process is already running.",
    backendUnexpectedExit: "The Person-Trading backend exited unexpectedly (code {code}).",
    ...
    backendExitedEarly: "The Person-Trading backend exited before it was ready (code {code}).\n{details}",
    backendHealthTimeout: "Timed out waiting for the Personal-Trading backend.\n{details}",
    backendNotFound: "Could not find person-trading.exe. Install the backend first or set VIBE_TRADING_EXECUTABLE.",
    ...
    loadingTitle: "Starting Personal-Trading",
  ...
  "zh-CN": {
    startupFailureTitle: "个人交易智能体启动失败",
    backendAlreadyRunning: "后端进程已经在运行。",
    backendUnexpectedExit: "个人交易智能体后端意外退出（代码 {code}）。",
    backendStarting: "后端已启动，正在等待健康检查 · {port}",
    backendReady: "本地服务已就绪 · {port}",
    backendNotStarted: "后端尚未启动。",
    backendExitedEarly: "个人交易智能体后端在就绪前退出（代码 {code}）。\n{details}",
    backendHealthTimeout: "等待 个人交易智能体后端就绪超时。\n{details}",
    backendNotFound: "找不到 person-trading.exe。请先安装后端，或设置 VIBE_TRADING_EXECUTABLE。",
  ...
  ja: {
    startupFailureTitle: "Personal-Trading の起動に失敗しました",
    backendAlreadyRunning: "バックエンドプロセスはすでに実行中です。",
    backendUnexpectedExit: "Personal-Trading バックエンドが予期せず終了しました（コード {code}）。",
    backendStarting: "バックエンドを起動しました。ヘルスチェックを待っています · {port}",
    backendReady: "ローカルサービスの準備ができました · {port}",
    backendNotStarted: "バックエンドはまだ起動していません。",
    backendExitedEarly: "Personal-Trading バックエンドが準備完了前に終了しました（コード {code}）。\n{details}",
    backendHealthTimeout: "Personal-Trading バックエンドの準備待ちがタイムアウトしました。\n{details}",
    backendNotFound: "person-trading.exe が見つかりません。バックエンドをインストールするか、VIBE_TRADING_EXECUTABLE を設定してください。",
    ...
    loadingTitle: "Personal-Trading を起動しています",
  ...
  ko: {
    startupFailureTitle: "Personal-Trading를 시작하지 못했습니다",
    backendAlreadyRunning: "백엔드 프로세스가 이미 실행 중입니다.",
    backendUnexpectedExit: "Personal-Trading 백엔드가 예기치 않게 종료되었습니다(코드 {code}).",
    backendStarting: "백엔드가 시작되었습니다. 상태 확인을 기다리는 중 · {port}",
    backendReady: "로컬 서비스가 준비되었습니다 · {port}",
    backendNotStarted: "백엔드가 아직 시작되지 않았습니다.",
    backendExitedEarly: "Personal-Trading 백엔드가 준비되기 전에 종료되었습니다(코드 {code}).\n{details}",
    backendHealthTimeout: "Personal-Trading 백엔드 준비를 기다리는 동안 시간이 초과되었습니다.\n{details}",
    backendNotFound: "person-trading.exe를 찾을 수 없습니다. 백엔드를 설치하거나 VIBE_TRADING_EXECUTABLE을 설정하세요.",
    ...
    loadingTitle: "Personal-Trading 시작 중",
  ...
  ar: {
    startupFailureTitle: "تعذّر بدء Personal-Trading",
    backendAlreadyRunning: "عملية الواجهة الخلفية قيد التشغيل بالفعل.",
    backendUnexpectedExit: "توقفت الواجهة الخلفية لـ Personal-Trading بشكل غير متوقع (الرمز {code}).",
    ...
    backendExitedEarly: "توقفت الواجهة الخلفية لـ Personal-Trading قبل أن تصبح جاهزة (الرمز {code}).\n{details}",
    backendHealthTimeout: "انتهت مهلة انتظار جاهزية الواجهة الخلفية لـ Personal-Trading.\n{details}",
    backendNotFound: "تعذّر العثور على person-trading.exe. ثبّت الواجهة الخلفية أولاً أو اضبط VIBE_TRADING_EXECUTABLE.",
    ...
    loadingTitle: "جارٍ بدء Personal-Trading",
...
export function resolveDesktopLocale(rawLocale: string | undefined): DesktopLocale {
  ...
  if (normalized === "zh" || normalized.startsWith("zh-cn") || normalized.startsWith("zh-hans") || normalized.startsWith("zh")) {
    return "zh-CN";
  }
  if (normalized === "ja" || normalized.startsWith("ja-")) return "ja";
  if (normalized === "ko" || normalized.startsWith("ko-")) return "ko";
  if (normalized === "ar" || normalized.startsWith("ar-")) return "ar";
  // Product default is Chinese, including English OS locales.
  return "zh-CN";
}
