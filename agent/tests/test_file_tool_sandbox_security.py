...
def test_tilde_expansion_resolves_to_mock_home(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("VIBE_TRADING_ALLOWED_WRITE_ROOTS", str(mock_home / ".person-trading"))
    ...
    resolved = resolve_safe_path("~/.person-trading/scripts/strat.py", None, allowed_write, purpose="write")
    assert resolved == mock_home / ".person-trading" / "scripts" / "strat.py"
