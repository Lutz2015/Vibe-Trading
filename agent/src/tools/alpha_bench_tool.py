...
Cache integrity note: the universe panel cache lives in ``~/.person-trading/cache/``
...
def _load_universe_panel(
    ...
    Args:
        ...
        use_cache: When True (default) reuse a pickle in
            ``~/.person-trading/cache/`` if the same universe+period was fetched
    ...
    cache_dir = Path.home() / ".person-trading" / "cache"
...
def _fetch_sp500_constituents() -> tuple[list[str], dict[str, str]]:
    ...
    try:
        ...
        resp = requests.get(
            ...
            headers={
                "User-Agent": (
                    "Person-Trading/0.1 (research bench; "
                    "https://github.com/HKUDS/Person-Trading)"
                )
            },
            timeout=20,
        )
...
def _default_output_dir() -> Path:
    return Path.home() / ".person-trading" / "reports"
...
class AlphaBenchTool(BaseTool):
    ...
    parameters = {
        ...
        "properties": {
            ...
            "output_dir": {
                "type": "string",
                "description": "Where to write the HTML report; default ~/.person-trading/reports/.",
            },
        },
