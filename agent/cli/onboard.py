...
Triggered automatically when ``~/.person-trading/.env`` does not exist. Five
back-steppable steps (provider → model → key → timeout → optional Tushare),
matching §3.2 of the 2026-05-19 UI/UX design proposal.

Each step persists immediately to ``~/.person-trading/.env.partial`` and the
...
def _env_dir() -> Path: return Path.home() / ".person-trading"
...
def _intro_header(console: Console) -> None:
    console.print()
    console.print(Text("  Person-Trading setup", style=Theme.primary))
    console.print(Text(
        "  We didn't find a config at ~/.person-trading/.env.\n"
    ...
    def step_key() -> object:
        ...
        if provider.key_env is None:
            ...
            if provider.key == "openai-codex":
                ...
                cons.print(Text(
                    "  After setup, run: person-trading provider login openai-codex",
                    style=Theme.muted,
                ))
            else:
        ...
        while True:
            key = _prompt_secret(
                ...
                "(saved to ~/.person-trading/.env, never logged)",
                console=cons,
            )
